...
import com.employee.performance.entity.EvaluationCycle;
// 行注释：导入业务服务类，用于复用核心业务逻辑。
import com.employee.performance.service.EvaluationCycleService;
...
// 类注释：这是 EvaluationCycleController 模块的控制器类，用于接收前端请求并调用服务层返回接口结果。
...
public class EvaluationCycleController {
    ...
    public ResponseEntity<List<EvaluationCycle>> getAllCycles() {
        // 行注释：返回 HTTP 响应对象给前端。
        return ResponseEntity.ok(evaluationCycleService.getAllCycles());
    // 行注释：结束当前代码块。
    }
    ...
    public ResponseEntity<EvaluationCycle> getCycleById(@PathVariable Long id) {
        // 行注释：返回 HTTP 响应对象给前端。
        return ResponseEntity.ok(evaluationCycleService.getCycleById(id));
    // 行注释：结束当前代码块。
    }
    ...
    public ResponseEntity<EvaluationCycle> createCycle(@RequestBody EvaluationCycle cycle) {
        // 行注释：返回 HTTP 响应对象给前端。
    ...
    public ResponseEntity<EvaluationCycle> updateCycle(@PathVariable Long id, @RequestBody EvaluationCycle cycle) {
        // 行注释：返回 HTTP 响应对象给前端。
        return ResponseEntity.ok(evaluationCycleService.updateCycle(id, cycle));
    // 行注释：结束当前代码块。
    }
    ...
    public ResponseEntity<Void> deleteCycle(@PathVariable Long id) {
        ...
        evaluationCycleService.deleteCycle(id);
        // 行注释：返回 HTTP 响应对象给前端。
    ...
    public ResponseEntity<List<EvaluationCycle>> getActiveCycles() {
        // 行注释：返回 HTTP 响应对象给前端。
        return ResponseEntity.ok(evaluationCycleService.getActiveCycles());
    // 行注释：结束当前代码块。
    }
// 行注释：结束当前代码块。
}
