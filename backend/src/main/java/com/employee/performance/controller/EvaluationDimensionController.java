...
import com.employee.performance.service.EvaluationDimensionService;
...
// 类注释：这是 EvaluationDimensionController 模块的控制器类，用于接收前端请求并调用服务层返回接口结果。
// 行注释：声明这是 REST 接口控制器，返回 JSON 数据给前端。
@RestController
// 行注释：配置当前控制器或方法对应的访问路径。
@RequestMapping("/api/evaluation-dimensions")
...
public class EvaluationDimensionController {
    ...
    @GetMapping
    // 行注释：声明成员变量，用于保存依赖对象或实体字段数据。
    public ResponseEntity<List<EvaluationDimension>> getAllDimensions(
            // 行注释：使用 @RequestParam 注解声明框架行为或元数据。
        ...
        if (positionType != null && enabled != null) {
            // 行注释：当前行参与完成该模块的业务逻辑。
            dimensions = evaluationDimensionService.findByEnabled(enabled).stream()
                    // 行注释：读取对象字段值，用于条件判断或组装返回结果。
                    .filter(d -> positionType.equals(d.getPositionType()))
                    // 行注释：执行当前语句，完成一次赋值、调用或配置操作。
                    .toList();
        // 行注释：开始一个代码块，下面是该结构内部的具体逻辑。
        } else if (positionType != null) {
            // 行注释：执行当前语句，完成一次赋值、调用或配置操作。
            dimensions = evaluationDimensionService.findByPositionType(positionType);
        // 行注释：开始一个代码块，下面是该结构内部的具体逻辑。
        } else if (enabled != null) {
            // 行注释：执行当前语句，完成一次赋值、调用或配置操作。
            dimensions = evaluationDimensionService.findByEnabled(enabled);
        // 行注释：开始一个代码块，下面是该结构内部的具体逻辑。
        } else {
            // 行注释：执行当前语句，完成一次赋值、调用或配置操作。
            dimensions = evaluationDimensionService.findAll();
        // 行注释：结束当前代码块。
        }
    ...
    public ResponseEntity<EvaluationDimension> getDimensionById(@PathVariable Long id) {
        // 行注释：返回 HTTP 响应对象给前端。
        return ResponseEntity.ok(evaluationDimensionService.findById(id));
    // 行注释：结束当前代码块。
    }
    ...
    public ResponseEntity<EvaluationDimension> createDimension(@RequestBody EvaluationDimension dimension) {
        // 行注释：返回 HTTP 响应对象给前端。
        return ResponseEntity.ok(evaluationDimensionService.save(dimension));
    // 行注释：结束当前代码块。
    }
    ...
    @PutMapping("/{id}")
    // 行注释：声明成员变量，用于保存依赖对象或实体字段数据。
    public ResponseEntity<EvaluationDimension> updateDimension(
            // 行注释：使用 @PathVariable Long id, 注解声明框架行为或元数据。
            @PathVariable Long id,
            // 行注释：使用 @RequestBody EvaluationDimension dimension) { 注解声明框架行为或元数据。
    ...
    public ResponseEntity<Void> deleteDimension(@PathVariable Long id) {
        // 行注释：执行当前语句，完成一次赋值、调用或配置操作。
        evaluationDimensionService.deleteById(id);
        // 行注释：返回 HTTP 响应对象给前端。
