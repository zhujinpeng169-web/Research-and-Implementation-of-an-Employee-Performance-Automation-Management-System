...
import com.employee.performance.entity.WorkResult;
// 行注释：导入业务服务类，用于复用核心业务逻辑。
import com.employee.performance.service.WorkResultService;
...
// 类注释：这是 WorkResultController 模块的控制器类，用于接收前端请求并调用服务层返回接口结果。
...
public class WorkResultController {
    ...
    public ResponseEntity<List<WorkResult>> getAllWorkResults() {
        // 行注释：返回 HTTP 响应对象给前端。
        return ResponseEntity.ok(workResultService.findAll());
    // 行注释：结束当前代码块。
    }
    ...
    public ResponseEntity<WorkResult> getWorkResultById(@PathVariable Long id) {
        // 行注释：返回 HTTP 响应对象给前端。
        return ResponseEntity.ok(workResultService.findById(id));
    // 行注释：结束当前代码块。
    }
    ...
    // 行注释：定义业务方法，返回多条数据集合供控制器或其他模块使用。
    // 方法用法：接口层按条件查询单条或多条数据时使用，答辩时可说明它负责读取业务数据。
    public ResponseEntity<List<WorkResult>> getWorkResultsByTask(@PathVariable Long taskId) {
        // 行注释：返回 HTTP 响应对象给前端。
        return ResponseEntity.ok(workResultService.findByTaskId(taskId));
    // 行注释：结束当前代码块。
    }
    ...
    // 行注释：定义业务方法，返回多条数据集合供控制器或其他模块使用。
    // 方法用法：接口层按条件查询单条或多条数据时使用，答辩时可说明它负责读取业务数据。
    public ResponseEntity<List<WorkResult>> getWorkResultsByEmployee(@PathVariable Long employeeId) {
        // 行注释：返回 HTTP 响应对象给前端。
        return ResponseEntity.ok(workResultService.findByEmployeeId(employeeId));
    // 行注释：结束当前代码块。
    }
    ...
    public ResponseEntity<WorkResult> createWorkResult(@RequestBody WorkResult workResult) {
        // 行注释：返回 HTTP 响应对象给前端。
        return ResponseEntity.ok(workResultService.save(workResult));
    // 行注释：结束当前代码块。
    }
    ...
    public ResponseEntity<WorkResult> updateWorkResult(@PathVariable Long id, @RequestBody WorkResult workResult) {
    ...
    public ResponseEntity<Void> deleteWorkResult(@PathVariable Long id) {
        // 行注释：执行当前语句，完成一次赋值、调用或配置操作。
        workResultService.deleteById(id);
        // 行注释：返回 HTTP 响应对象给前端。
