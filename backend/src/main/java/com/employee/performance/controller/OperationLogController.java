...
// 类注释：这是 OperationLogController 模块的控制器类，用于接收前端请求并调用服务层返回接口结果。
...
public class OperationLogController {
    ...
    public ResponseEntity<List<OperationLog>> getAllLogs(
        ...
        if (module != null && operation != null) {
            // 行注释：执行当前语句，完成一次赋值、调用或配置操作。
            logs = operationLogService.findByModuleAndOperation(module, operation);
        // 行注释：开始一个代码块，下面是该结构内部的具体逻辑。
        } else if (module != null) {
            // 行注释：执行当前语句，完成一次赋值、调用或配置操作。
            logs = operationLogService.findByModule(module);
        // 行注释：开始一个代码块，下面是该结构内部的具体逻辑。
        } else if (operation != null) {
            // 行注释：执行当前语句，完成一次赋值、调用或配置操作。
            logs = operationLogService.findByOperation(operation);
        // 行注释：开始一个代码块，下面是该结构内部的具体逻辑。
        } else {
            // 行注释：执行当前语句，完成一次赋值、调用或配置操作。
    ...
    public ResponseEntity<OperationLog> getLogById(@PathVariable Long id) {
        // 行注释：返回 HTTP 响应对象给前端。
        return ResponseEntity.ok(operationLogService.findById(id));
    // 行注释：结束当前代码块。
    }
    ...
    // 方法用法：接口层按条件查询单条或多条数据时使用，答辩时可说明它负责读取业务数据。
    public ResponseEntity<List<OperationLog>> getLogsByUser(@PathVariable Long userId) {
        // 行注释：返回 HTTP 响应对象给前端。
        return ResponseEntity.ok(operationLogService.findByUserId(userId));
    // 行注释：结束当前代码块。
    }
    ...
    public ResponseEntity<OperationLog> createLog(@RequestBody OperationLog log) {
        // 行注释：返回 HTTP 响应对象给前端。
        return ResponseEntity.ok(operationLogService.save(log));
    // 行注释：结束当前代码块。
    }
