...
import com.employee.performance.service.ImprovementPlanService;
...
// 类注释：这是 ImprovementPlanController 模块的控制器类，用于接收前端请求并调用服务层返回接口结果。
...
public class ImprovementPlanController {
    ...
    public ResponseEntity<List<ImprovementPlan>> getAllPlans() {
        // 行注释：返回 HTTP 响应对象给前端。
        return ResponseEntity.ok(improvementPlanService.getAllPlans());
    // 行注释：结束当前代码块。
    }
    ...
    public ResponseEntity<ImprovementPlan> getPlanById(@PathVariable Long id) {
        // 行注释：返回 HTTP 响应对象给前端。
        return ResponseEntity.ok(improvementPlanService.getPlanById(id));
    // 行注释：结束当前代码块。
    }
    ...
    public ResponseEntity<ImprovementPlan> createPlan(@RequestBody ImprovementPlan plan) {
        // 行注释：返回 HTTP 响应对象给前端。
        return ResponseEntity.ok(improvementPlanService.createPlan(plan));
    // 行注释：结束当前代码块。
    }
    ...
    public ResponseEntity<ImprovementPlan> updatePlan(@PathVariable Long id, @RequestBody ImprovementPlan plan) {
        // 行注释：返回 HTTP 响应对象给前端。
        return ResponseEntity.ok(improvementPlanService.updatePlan(id, plan));
    // 行注释：结束当前代码块。
    }
    ...
    public ResponseEntity<Void> deletePlan(@PathVariable Long id) {
        ...
        improvementPlanService.deletePlan(id);
        // 行注释：返回 HTTP 响应对象给前端。
    ...
    // 行注释：定义业务方法，返回多条数据集合供控制器或其他模块使用。
    // 方法用法：接口层按条件查询单条或多条数据时使用，答辩时可说明它负责读取业务数据。
    public ResponseEntity<List<ImprovementPlan>> getPlansByEmployee(@PathVariable Long employeeId) {
        // 行注释：返回 HTTP 响应对象给前端。
        return ResponseEntity.ok(improvementPlanService.getPlansByEmployee(employeeId));
    // 行注释：结束当前代码块。
    }
// 行注释：结束当前代码块。
}
