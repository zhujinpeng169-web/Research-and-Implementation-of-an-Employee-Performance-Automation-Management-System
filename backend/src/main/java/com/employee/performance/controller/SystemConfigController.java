...
import com.employee.performance.service.SystemConfigService;
...
// 类注释：这是 SystemConfigController 模块的控制器类，用于接收前端请求并调用服务层返回接口结果。
...
public class SystemConfigController {
    ...
    public ResponseEntity<SystemConfig> createConfig(@RequestBody SystemConfig config) {
        // 行注释：返回 HTTP 响应对象给前端。
        return ResponseEntity.ok(systemConfigService.save(config));
    // 行注释：结束当前代码块。
    }
