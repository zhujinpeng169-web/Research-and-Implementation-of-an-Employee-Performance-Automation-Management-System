...
import com.employee.performance.repository.EvaluationCycleRepository;
...
// 类注释：这是 EvaluationCycleService 模块的业务服务类，用于承载核心业务逻辑并协调数据读写。
...
public class EvaluationCycleService {
    ...
    public List<EvaluationCycle> getAllCycles() {
        // 行注释：返回当前方法的处理结果。
        return evaluationCycleRepository.findAll();
    // 行注释：结束当前代码块。
    }
    ...
    public EvaluationCycle getCycleById(Long id) {
        // 行注释：返回当前方法的处理结果。
        return evaluationCycleRepository.findById(id).orElse(null);
    // 行注释：结束当前代码块。
    }
    ...
    public EvaluationCycle createCycle(EvaluationCycle cycle) {
        ...
        // 行注释：给实体对象字段赋值，用于新增或更新业务数据。
        cycle.setStatus(cycle.getStatus() != null ? cycle.getStatus() : "pending");
        // 行注释：返回当前方法的处理结果。
    ...
    public EvaluationCycle updateCycle(Long id, EvaluationCycle cycle) {
        ...
        if (existing != null) {
            ...
            // 行注释：返回当前方法的处理结果。
            return evaluationCycleRepository.save(existing);
        // 行注释：结束当前代码块。
        }
        // 行注释：返回当前方法的处理结果。
        return null;
    ...
    public void deleteCycle(Long id) {
        // 行注释：执行当前语句，完成一次赋值、调用或配置操作。
        evaluationCycleRepository.deleteById(id);
    // 行注释：结束当前代码块。
    }

    // 行注释：定义业务方法，返回多条数据集合供控制器或其他模块使用。
    public List<EvaluationCycle> getActiveCycles() {
        // 行注释：返回当前方法的处理结果。
        return evaluationCycleRepository.findByStatus("active");
    // 行注释：结束当前代码块。
    }
// 行注释：结束当前代码块。
}
