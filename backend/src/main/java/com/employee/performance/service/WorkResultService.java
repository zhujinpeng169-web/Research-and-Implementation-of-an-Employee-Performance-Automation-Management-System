...
import com.employee.performance.repository.WorkResultRepository;
...
// 类注释：这是 WorkResultService 模块的业务服务类，用于承载核心业务逻辑并协调数据读写。
...
public class WorkResultService {
    ...
    public List<WorkResult> findAll() {
        // 行注释：返回当前方法的处理结果。
    ...
    public WorkResult findById(Long id) {
        // 行注释：返回当前方法的处理结果。
        return workResultRepository.findById(id)
                // 行注释：执行当前语句，完成一次赋值、调用或配置操作。
                .orElseThrow(() -> new RuntimeException("Work result not found"));
    // 行注释：结束当前代码块。
    }

    // 行注释：定义业务方法，返回多条数据集合供控制器或其他模块使用。
    // 方法用法：业务层按条件查询单条或多条数据时使用，答辩时可说明它负责读取业务数据。
    public List<WorkResult> findByTaskId(Long taskId) {
        // 行注释：返回当前方法的处理结果。
        return workResultRepository.findByTaskId(taskId);
    // 行注释：结束当前代码块。
    }

    // 行注释：定义业务方法，返回多条数据集合供控制器或其他模块使用。
    public List<WorkResult> findByEmployeeId(Long employeeId) {
        // 行注释：返回当前方法的处理结果。
        return workResultRepository.findByEmployeeId(employeeId);
    // 行注释：结束当前代码块。
    }

    // 行注释：定义业务方法，封装一段可复用的处理逻辑。
    // 方法用法：业务层新增或保存数据时使用，负责把前端提交的业务对象持久化。
    public WorkResult save(WorkResult workResult) {
        // 行注释：进行条件判断，根据业务状态决定后续处理分支。
        if (workResult.getId() == null) {
            // 行注释：给实体对象字段赋值，用于新增或更新业务数据。
            workResult.setCreatedAt(LocalDateTime.now());
        // 行注释：结束当前代码块。
        }
        // 行注释：返回当前方法的处理结果。
