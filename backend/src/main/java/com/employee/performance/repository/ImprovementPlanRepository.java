...
// 类注释：这是 ImprovementPlanRepository 模块的数据访问接口，Spring Data JPA 会根据接口与方法命名规则自动提供增删改查能力。
...
public interface ImprovementPlanRepository extends JpaRepository<ImprovementPlan, Long> {
    ...
    List<ImprovementPlan> findByEmployeeId(Long employeeId);
    // 行注释：执行当前语句，完成一次赋值、调用或配置操作。
    List<ImprovementPlan> findByStatus(String status);
// 行注释：结束当前代码块。
}
