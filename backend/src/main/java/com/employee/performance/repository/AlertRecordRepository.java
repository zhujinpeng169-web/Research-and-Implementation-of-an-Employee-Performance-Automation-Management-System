...
// 类注释：这是 AlertRecordRepository 模块的数据访问接口，Spring Data JPA 会根据接口与方法命名规则自动提供增删改查能力。
// 行注释：定义当前模块的核心类型，是该文件对外提供功能的主体。
public interface AlertRecordRepository extends JpaRepository<AlertRecord, Long> {
    // 行注释：执行当前语句，完成一次赋值、调用或配置操作。
    // 方法用法：数据访问层按 TargetIdAndTargetType 条件查询数据，Spring Data JPA 会根据方法名自动生成 SQL。
    List<AlertRecord> findByTargetIdAndTargetType(Long targetId, String targetType);
    // 行注释：执行当前语句，完成一次赋值、调用或配置操作。
