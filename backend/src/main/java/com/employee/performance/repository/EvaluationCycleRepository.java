...
// 类注释：这是 EvaluationCycleRepository 模块的数据访问接口，Spring Data JPA 会根据接口与方法命名规则自动提供增删改查能力。
...
public interface EvaluationCycleRepository extends JpaRepository<EvaluationCycle, Long> {
    ...
    // 方法用法：数据访问层按 Status 条件查询数据，Spring Data JPA 会根据方法名自动生成 SQL。
    List<EvaluationCycle> findByStatus(String status);
    // 行注释：执行当前语句，完成一次赋值、调用或配置操作。
    List<EvaluationCycle> findByType(String type);
// 行注释：结束当前代码块。
}
